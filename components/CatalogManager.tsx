...
export const CatalogManager: React.FC<CatalogManagerProps> = ({ config, setConfig, catalog, setCatalog }) => {
  ...
  const addModel = () => {
    const newModel: QuoteCard = {
      id: 'mod-' + Date.now(),
      type: 'AIRCRAFT',
      title: { [Language.FR]: 'Nouveau Modèle', [Language.EN]: '', [Language.DE]: '', [Language.IT]: '', [Language.ES]: '', [Language.RU]: '', [Language.HU]: '', [Language.PL]: '' },
      description: { [Language.FR]: '', [Language.EN]: '', [Language.DE]: '', [Language.IT]: '', [Language.ES]: '', [Language.RU]: '', [Language.HU]: '', [Language.PL]: '' },
      images: ['', '', ''],
      price: 0,
      isOptional: false,
      quantity: 1
  ...
  const deleteModel = (id: string) => {
    if (confirm("Supprimer ce modèle du catalogue ?")) {
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-12">
      <div className="flex gap-4 bg-white p-2 rounded-2xl shadow-sm border w-fit no-print">
        <button onClick={() => setTab('BRANDING')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition ${tab === 'BRANDING' ? 'bg-slate-900 text-white' : 'text-slate-400 hover:bg-slate-50'}`}>Société & Branding</button>
        <button onClick={() => setTab('CATALOG')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition ${tab === 'CATALOG' ? 'bg-slate-900 text-white' : 'text-slate-400 hover:bg-slate-50'}`}>Catalogue Master</button>
      </div>
      ...
      {tab === 'BRANDING' ? (
        <div className="animate-in fade-in slide-in-from-bottom-4 space-y-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="bg-white p-10 rounded-[32px] shadow-xl border border-slate-100 space-y-8">
              <h3 className="text-2xl font-serif italic text-slate-800 border-l-8 pl-6" style={{ borderColor: config.primaryColor }}>Identité Visuelle</h3>
              <div className="space-y-6">
                 <div>
                    ...
                    <div className="mt-4 p-8 border-2 border-dashed border-slate-100 rounded-[2.5rem] flex items-center justify-center bg-slate-50 relative group overflow-hidden">
                      ...
                      <label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center cursor-pointer">
                        ...
                        <svg className="w-8 h-8 text-white mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-5-7l-5-5m0 0l-5 5m5-5v12"/></svg>
                 ...
                 <div>
                    ...
                    <div className="flex gap-4">
                      <input type="color" value={config.primaryColor} onChange={(e) => setConfig({...config, primaryColor: e.target.value})} className="h-16 w-24 cursor-pointer rounded-2xl border-none p-0 overflow-hidden" />
                      <input type="text" value={config.primaryColor} onChange={(e) => setConfig({...config, primaryColor: e.target.value})} className="flex-1 bg-slate-50 rounded-2xl px-6 font-black uppercase text-sm border-none focus:ring-2 focus:ring-slate-200" />
                    </div>
                 </div>
                 <div className="space-y-4 pt-4 border-t">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-300 uppercase ml-2">Mentions Légales (Gauche)</label>
                      <textarea value={config.legalDisclaimer} onChange={(e) => setConfig({...config, legalDisclaimer: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-xs h-24 border-none resize-none" placeholder="Ex: Broker de vols, transporteurs certifiés..." />
                    </div>
                 </div>
              </div>
            </div>
            <div className="bg-white p-10 rounded-[32px] shadow-xl border border-slate-100 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input type="text" placeholder="Clé API" value={config.googleMapsApiKey || ''} onChange={(e) => setConfig({...config, googleMapsApiKey: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-sm font-bold border-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-300 uppercase ml-2">Adresse Siège</label>
                  <input type="text" placeholder="Dubai Airport..." value={config.address} onChange={(e) => setConfig({...config, address: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-sm font-bold border-none" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-300 uppercase ml-2">Téléphone</label>
                    <input type="text" placeholder="+971..." value={config.phone} onChange={(e) => setConfig({...config, phone: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-sm font-bold border-none" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-300 uppercase ml-2">Email</label>
                    <input type="text" placeholder="booking@..." value={config.email} onChange={(e) => setConfig({...config, email: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-sm font-bold border-none" />
                  </div>
                </div>
                <input type="text" placeholder="Site Web" value={config.website} onChange={(e) => setConfig({...config, website: e.target.value})} className="w-full bg-slate-50 rounded-2xl p-4 text-sm font-bold border-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-[32px] shadow-xl border border-slate-100 space-y-8">
            <div className="flex justify-between items-center">
              ...
              <button onClick={addAgent} className="bg-slate-900 text-white px-6 py-2 rounded-full text-[10px] font-black uppercase">Ajouter un agent</button>
            </div>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex justify-between items-center">
            <h2 className="text-3xl font-serif italic text-slate-800">Modèles <span className="text-[#d4af37]">LalaJet</span></h2>
            <div className="flex gap-4">
               <div className="flex bg-slate-100 p-1 rounded-full">
                 {LANGUAGES.map(l => (
                   <button key={l.code} onClick={() => setEditingLang(l.code)} className={`px-4 py-1.5 rounded-full text-[10px] font-black transition ${editingLang === l.code ? 'bg-white shadow-sm text-black' : 'text-slate-400'}`}>{l.code}</button>
                 ))}
               </div>
               <button onClick={addModel} className="bg-slate-900 text-[#d4af37] border border-[#d4af37] px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl transition hover:bg-[#d4af37] hover:text-white">Ajouter au Catalogue</button>
            </div>
          </div>
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                  <div className="lg:w-1/3 space-y-4">
                    <div className="aspect-video bg-slate-100 rounded-[2rem] overflow-hidden relative group/img border border-slate-100">
                      {model.images[0] ? <img src={model.images[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 text-[10px] font-black uppercase tracking-widest">Image Principale</div>}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       {[1, 2].map(imgIdx => (
                         <div key={imgIdx} className="aspect-square bg-slate-50 rounded-2xl overflow-hidden relative group/subimg border border-slate-100">
                           {model.images[imgIdx] ? <img src={model.images[imgIdx]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-200 text-[8px] font-black uppercase">Détail</div>}
                  ...
                  <div className="flex-1 space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1">
                        ...
                        <input type="text" value={model.title[editingLang] || ''} onChange={(e) => {
                          const newCat = [...catalog];
                      ...
                      <div className="space-y-1">
                        ...
                        <input type="number" value={model.price} onChange={(e) => updateModel(idx, { price: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-50 rounded-2xl p-4 text-xl font-black border-none" />
                      </div>
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-300 uppercase ml-2">Description ({editingLang})</label>
                      <textarea value={model.description[editingLang] || ''} onChange={(e) => {
                        const newCat = [...catalog];
                        newCat[idx].description[editingLang] = e.target.value;
                        setCatalog(newCat);
                      }} className="w-full bg-slate-50 rounded-[2rem] p-6 text-sm italic font-medium h-40 border-none resize-none focus:ring-2 focus:ring-[#d4af37]/20" placeholder="Décrivez l'avion, sa configuration, son autonomie..." />
                    </div>
                    <div className="flex gap-4 items-center">
                       <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Type d'offre :</span>
                       <select value={model.type} onChange={(e) => updateModel(idx, { type: e.target.value as any })} className="bg-slate-100 rounded-full px-6 py-2 text-[10px] font-black uppercase border-none">
