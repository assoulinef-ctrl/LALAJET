...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Si Supabase est présent, on tente une synchronisation
    if (supabase) {
      fetchFromSupabase();
    }
  }, []);
  ...
  const fetchFromSupabase = async () => {
    if (!supabase) return;
    try {
      // 1. Chargement des Devis
      const { data: quotesData } = await supabase.from('quotes').select('data');
      if (quotesData && quotesData.length > 0) {
        const remoteQuotes = quotesData.map(q => q.data);
        setArchives(prev => {
          const combined = [...prev];
          remoteQuotes.forEach((rq: Quote) => {
      ...
      if (clientsData && clientsData.length > 0) {
        const remoteClients = clientsData.map(c => c.data);
        setClients(prev => {
          const combined = [...prev];
          remoteClients.forEach((rc: Client) => {
      ...
      // 3. Chargement du Catalogue (Priorité Supabase si non vide)
      ...
      if (catError) {
        ...
        const remoteCat = catData.map(c => c.data);
        setCatalog(remoteCat);
        console.log(`Catalog loaded from Supabase: ${remoteCat.length} items`);
      }
    } catch (err) {
  ...
  // Sauvegarde locale automatique (Persistence localStorage)
  useEffect(() => {
  ...
  const saveToArchive = async () => {
    console.log("[LalaJet] SAVE clicked");
    setDbStatus("SAVE clicked");
    try {
      ...
      if (supabase) {
        setDbStatus(`Supabase: ON | DB SYNC started...`);
        console.log("[LalaJet] DB SYNC started");
        ...
        // 3. Sauvegarde Catalog (Upsert unitaire pour respecter l'ID de l'article)
        if (catalog && catalog.length > 0) {
          const catPromises = catalog.map(item => {
            // Garantir un ID stable s'il manquait par hasard
            const itemId = item.id || `card-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
          ...
          const catErrors = catResults.filter(r => r.error);
          if (catErrors.length > 0) {
            console.error("[LalaJet] Catalog upsert error:", catErrors[0].error?.message);
          } else {
            console.log(`Catalog upsert to Supabase: ${catalog.length} items`);
          }
        }

        if (quoteError) {
          console.error("[LalaJet] DB INSERT error:", quoteError.message);
          setDbStatus(`Supabase: ERROR (${quoteError.message})`);
        } else {
          console.log("[LalaJet] DB SYNC success");
          setDbStatus(`Supabase: ON | DB SYNC success`);
        }
      } else {
        console.warn("[LalaJet] Supabase OFF (env missing)");
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-[#E73188]/10">
      <nav className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-10 sticky top-0 z-[60] shadow-sm no-print">
        ...
        <div className="flex items-center gap-4">
           {view === 'EDITOR' && (
             <>
               {dbStatus && <span className="text-[8px] font-black uppercase tracking-widest text-[#E73188] animate-pulse bg-[#E73188]/5 px-3 py-1 rounded-full">{dbStatus}</span>}
      ...
      <div className="fixed bottom-10 right-10 flex flex-col gap-2 no-print z-50">
        <div className="flex gap-2 mb-4">
          <button 
            ...
            className={`w-10 h-10 rounded-full bg-white shadow-xl border-2 flex items-center justify-center text-[10px] font-black hover:scale-110 transition ${activeQuote.currency === Currency.EUR ? 'border-[#d4af37]' : 'border-slate-100'}`}
          >€</button>
          <button 
            onClick={() => setActiveQuote(p => ({ ...p, currency: Currency.USD }))}
            className={`w-10 h-10 rounded-full bg-white shadow-xl border-2 flex items-center justify-center text-[10px] font-black hover:scale-110 transition ${activeQuote.currency === Currency.USD ? 'border-[#d4af37]' : 'border-slate-100'}`}
          >$</button>
          <button 
            onClick={() => setActiveQuote(p => ({ ...p, currency: Currency.AED }))}
            className={`w-10 h-10 rounded-full bg-white shadow-xl border-2 flex items-center justify-center text-[8px] font-black hover:scale-110 transition ${activeQuote.currency === Currency.AED ? 'border-[#d4af37]' : 'border-slate-100'}`}
          >AED</button>
        </div>
        <div className="flex gap-2">
          {LANGUAGES.map(l => (
            <button 
              ...
              className={`w-10 h-10 rounded-full bg-white shadow-xl border-2 flex items-center justify-center text-lg hover:scale-110 transition ${activeQuote.language === l.code ? 'border-[#E73188]' : 'border-slate-100'}`}
              title={l.name}
            >
              {l.flag}
            </button>
